...
export default function GetWalletFromSeed() {
    ...
    const fetchWallet = async () => {
        console.log(inputs)
        try{
            let mneumonic = inputs.toString().replace(/,/g, " ")
            let wallet = ethers.Wallet.fromPhrase(mneumonic);
            setWallet(wallet);
        ...
        <div className="w-full h-screen flex flex-col px-8 pt-14">
            ...
            <div className="w-full flex justify-center">
                <div className="relative w-full h-[250px] rounded-lg bg-gray-800 mt-10 overflow-hidden">
                    ...
                    <div className={`relative text-white text-[10px] p-2 flex flex-wrap justify-between items-center`}>

                        {
                            inputs?.map((val, index) => (
                                <div key={index} className="flex justify-center items-center w-20 h-10 rounded-lg border-white border-[1px] m-2">
                                    <input
                                        type="text"
            ...
            <div className="w-full flex flex-1 flex-col justify-end">
                <button onClick={fetchWallet} className="w-full flex items-center justify-center bg-gradient-to-r from-green-400 to-purple-400 py-2 px-4 mb-10 rounded-md text-white font-semibold">
                    Recover
                </button>
            </div>
