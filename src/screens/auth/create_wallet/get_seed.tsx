  ...
  return (
    <div className="w-full h-screen flex flex-col px-8 pt-14">
          ...
          {isBlured && (
            <div className="absolute inset-0 flex flex-col p-5 backdrop-blur-lg bg-transparent rounded-lg">
              <span className="text-xl font-bold text-white mt-4">
                Tap to reveal your Secret Phrase
              </span>
              <span className="text-sm text-gray-100 mt-2 font-semibold">
                Make sure no one is watching you.
              </span>

              <button
                onClick={() => setIsBlured(false)}
                className="w-full flex justify-center items-center border-gray-300 border-[1px] shadow-md h-12 px-4 mt-10 rounded-md"
              >
      ...
      <div className="w-full flex flex-1 flex-col justify-end">
        <button
          disabled={isBlured}
          onClick={() => navigate("/auth/set-wallet-password")}
